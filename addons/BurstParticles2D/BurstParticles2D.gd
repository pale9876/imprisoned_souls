...
const SHADER: Shader = preload("BurstParticleGradientMap.gdshader")
const SHADER_ADD: Shader = preload("BurstParticleGradientMapAdd.gdshader")
...
func _create_particle() -> int:
	...
	var p_rid: RID = RenderingServer.canvas_item_create()
...
func burst() -> void:
	...
	var timer: SceneTreeTimer = get_tree().create_timer(lifetime, false)
...
func _get_update_functions() -> Array[Callable]:
	...
	if x_scale_curve:
		update_functions.append(
			func(t: float, p_id: int) -> void: arr_x_scale[p_id] = x_scale_curve.sample_baked(t)
		)
