...
@export var background_color: Color = Color(0.081, 0.102, 0.11, 1.0)
...
func create_gradient(width: int = 256) -> GradientTexture1D:
...
func create() -> void:
	...
	RenderingServer.canvas_item_add_rect(
		background_cid, Rect2(Vector2(), Vector2(size)), background_color
	)
	...
	RenderingServer.canvas_item_add_texture_rect(
		generator_cid, Rect2(Vector2(), Vector2(float(size.x), float(size.y) / 3. * 2.)), gradient.get_rid()
	)
	RenderingServer.canvas_item_add_rect(
		generator_cid,
		Rect2(Vector2(0., float(size.y) / 3. * 2.), Vector2(float(size.x) / 2., float(size.y) / 3.)),
		Color.BLACK
	)
	RenderingServer.canvas_item_add_rect(
		generator_cid,
		Rect2(Vector2(float(size.x) / 2., float(size.y) / 3. * 2.), Vector2(float(size.x) / 2., float(size.y) / 3.)),
		Color.PURPLE
	)
	...
	RenderingServer.canvas_item_add_rect(
		guideline_cid, Rect2(Vector2(), size), Color(0.0, 0.0, 0.0, 0.0)
	)
	
	RenderingServer.canvas_item_set_transform(background_cid, Transform2D(0., - size / 2.))
